...
def serve_route_segment(route_list, start_zip, start_time, drive_used=0, duty_used=0, verbose=False):
    ...
    for stop in route_list:
        ...
        end_of_day = departure % 24
        before_close = (WINDOW_OPEN <= time_of_day <= WINDOW_CLOSE) and (end_of_day <= WINDOW_CLOSE)
...
def latest_break_transition(last_zip, first_next_zip, finish_time, drive_used, duty_used):
    ...
    if travel_needed_hours <= legal_travel_hours:
        ...
        break_start_time = arrival_B_day1          # take break immediately on arrival
        wait_at_B_day1 = 0.0
...
def evaluate_overnight_route(day1_route, day2_route, verbose=False):
    ...
    dispatch_time = max(0.0, WINDOW_OPEN - first_drive_day1)
...
def compute_savings(orders_df):
    """
    For every pair of stops (i, j), compute:
        s(i, j) = d(depot, i) + d(depot, j) - d(i, j)
    Returns a list of (savings, i_orderid, j_orderid) sorted descending.
    """
...
def clarke_wright(day_orders):
    ...
    We iterate savings in descending order and merge two routes when:
      - stop i is at the tail of one route AND stop j is at the head of another
        (or vice versa)
      - the merged route is feasible (capacity, windows, DOT)
      - the two routes are not already the same route
    """
    order_records = {int(row["ORDERID"]): row for _, row in day_orders.iterrows()}

    # Each route is a list of order dicts; keyed by route_id (arbitrary int)
    routes = {oid: [rec] for oid, rec in order_records.items()}

    # Track which route each order belongs to, and its position (head/tail)
    # route_of[oid] = route_id
    route_of = {oid: oid for oid in order_records}

    # head_of[route_id] = first order id in that route
    # tail_of[route_id] = last order id in that route
    head_of = {oid: oid for oid in order_records}
    tail_of = {oid: oid for oid in order_records}

    savings = compute_savings(day_orders)

    for s, oid_i, oid_j in savings:
        if s <= 0:
            break  # no more improving merges possible

        # Both orders must still exist in the data
        ...
        # Determine valid merge orientations:
        #   i at tail of its route, j at head of its route  -> route_i + route_j
        #   j at tail of its route, i at head of its route  -> route_j + route_i
        #   i at tail, j at tail -> route_i + reversed(route_j)
        #   i at head, j at head -> reversed(route_i) + route_j
        candidates = []
        ...
        for merged_route, keep_rid, drop_rid in candidates:
            result = evaluate_route(merged_route, verbose=False)
            if result["overall_feasible"]:
                # Commit the merge: store under keep_rid, remove drop_rid
                routes[keep_rid] = merged_route
                del routes[drop_rid]

                new_head = int(merged_route[0]["ORDERID"])
                new_tail = int(merged_route[-1]["ORDERID"])
                head_of[keep_rid] = new_head
                tail_of[keep_rid] = new_tail

                for stop in merged_route:
                    route_of[int(stop["ORDERID"])] = keep_rid

                # Clean up old head/tail entries for dropped route
                if drop_rid in head_of:
                    del head_of[drop_rid]
                if drop_rid in tail_of:
                    del tail_of[drop_rid]

                merged = True
                break  # only apply first feasible orientation
...
def or_opt_route(route):
    """
    Or-opt: try relocating single stops and chains of 2-3 stops
    to a better position within the same route.
    """
    best_route = route[:]
...
def find_all_overnight_candidates(routes_day1, routes_day2):
    """
...
def apply_overnight_improvements(routes_by_day):
    """
    Apply ALL non-conflicting overnight pairings (greedy by savings),
    not just the single best per day pair.
    """
    overnight_routes = []
    used_routes = {day: set() for day in routes_by_day}

    for d1, d2 in ADJACENT_DAY_PAIRS:
        candidates = find_all_overnight_candidates(routes_by_day[d1], routes_by_day[d2])
...
def print_overnight_candidates(routes_by_day):
    for d1, d2 in ADJACENT_DAY_PAIRS:
        candidates = find_all_overnight_candidates(routes_by_day[d1], routes_by_day[d2])
...
def main():
    ...
    for day in DAYS:
        ...
        routes = clarke_wright(day_orders)       # CW construction
        routes = consolidate_routes(routes)       # reduce van count
        routes = improve_routes(routes)           # 2-opt + or-opt
    ...
    print_overnight_candidates(routes_by_day)
