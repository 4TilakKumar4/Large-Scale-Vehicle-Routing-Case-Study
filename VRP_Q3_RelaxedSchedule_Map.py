...
COUNTRY_CODE = "US"
OUTPUT_FILE = "relaxed_routes_map.html"
...
def evaluate_route(routeList, verbose=False):
    ...
    for stop in routeList:
        ...
        if verbose:
            print(
                f"  Stop {int(stop['ORDERID'])}: arrive {to_clock(arrival)} | "
...
def solve_one_day(dayOrders):
    """Run the routing pipeline for one day."""
    if len(dayOrders) == 0:
        return []
...
def solve_schedule(orders):
    """Solve all five weekdays using the routing code."""
    routesByDay = {}
...
def get_visit_groups(orders, store_col="TOZIP"):
    ...
    Grouping by (store, day) preserves the number of weekly visits.
    """
    visitGroups = []
...
def relax_delivery_days(initialOrders, store_col="TOZIP", lambda_balance=25, max_passes=5, verbose=True):
    """
...
def check_schedule_feasibility(routesByDay):
    """Check every route in the weekly solution."""
...
def main_relaxed_schedule():
    ...
    check feasibility, then build a map of the relaxed routes.
    """
    ...
    m = build_map(relaxedRoutes, zipCoords)
    m.save(OUTPUT_FILE)
